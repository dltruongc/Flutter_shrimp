...
//    Map<String, dynamic> sys = weatherData['sys'];
    ...
    Map<String, dynamic> country = {'country': weatherData['sys']['country']};
    Map<String, dynamic> sunrise = {'sunrise': weatherData['sys']['sunrise']};
    Map<String, dynamic> sunset = {'sunset': weatherData['sys']['sunset']};

    _latitude = _unpackDouble(coord, 'lat');
    _longitude = _unpackDouble(coord, 'lon');

    _country = _unpackString(country, 'country');
    _sunrise = _unpackDate(sunrise, 'sunrise');
    _sunset = _unpackDate(sunset, 'sunset');
...
class WeatherStation {
  ...
  Future<List<Weather>> fiveDayForecast() async {
    ...
    try {
      ...
      forecasts = forecastsJson.map((w) {
        w.addAll(jsonForecasts['city']);
        return Weather(w);
      }).toList();
    } catch (exception) {
      print(exception);
  ...
  Future<bool> manageLocationPermission() async {
    ...
    bool ok = hasPermission == PermissionStatus.GRANTED;
    if (ok) {
      return true;
    } else {
      PermissionStatus permissionWasGranted =
          await location.requestPermission();
      bool wasOK = permissionWasGranted == PermissionStatus.GRANTED;
      return wasOK;
    }
  }
